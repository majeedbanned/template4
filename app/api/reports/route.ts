...
let title="";
    ...
    if (fromdate) {
      ...
      title+="["+" تا تاریخ: "+todate+"]"
        ...
        fromdateq = ` AND ((dbo.new_account.paidDate >= '${fromdate}'  and dbo.new_account.paidDate <= '${todate}'  ) 
                     OR (dbo.new_account.paidDate1 >= '${fromdate}' and dbo.new_account.paidDate1 <= '${todate}'  )) `;
      }
...
// console.log('x',novq, 'x',rahroq , 'x',bazarq,  'x',tabaghq,  'x',dateq, 
//  'x', activeq, 'x', searchq, 'x', pardakhtq,'x',  pardakhtqB,  'x',debtq,  'x',fromdateq, 'x', npardakhtq, 'x', shiveq, 'x', tajmiq )

//  console.log(novq ==="" , rahroq , bazarq==="" ,  tabaghq==="" ,  dateq==="" , 
