...
export interface OwnerSearchFilters {
  ...
  storePelak?: string;
  changeOwner?: string;
  changeOwnerFrom?: string;
  changeOwnerTo?: string;
...
interface OwnerSearchModalStore {
  isOpen: boolean;
  filters: OwnerSearchFilters;
  onOpen: () => void;
  onClose: () => void;
  onApply: (filters: OwnerSearchFilters) => void;
  setFilters: (filters: OwnerSearchFilters) => void;
}

const useOwnerSearchModal = create<OwnerSearchModalStore>((set) => ({
  isOpen: false,
  filters: {},
  onOpen: () => set({ isOpen: true }),
  onClose: () => set({ isOpen: false }),
  onApply: (filters: OwnerSearchFilters) => {
    set({ filters, isOpen: false });
  },
  setFilters: (filters: OwnerSearchFilters) => set({ filters }),
}));

export const OwnerSearchModal = ({
  onSearch,
}: {
  onSearch: (filters: OwnerSearchFilters) => void;
}) => {
  const ownerSearchModal = useOwnerSearchModal();
  const [localFilters, setLocalFilters] = useState<OwnerSearchFilters>({});

  useEffect(() => {
    if (ownerSearchModal.isOpen) {
      setLocalFilters(ownerSearchModal.filters);
    }
  }, [ownerSearchModal.isOpen, ownerSearchModal.filters]);

  const handleApply = () => {
    ownerSearchModal.onApply(localFilters);
    onSearch(localFilters);
    toast.success("جستجو اعمال شد");
  };

  const handleReset = () => {
    const emptyFilters: OwnerSearchFilters = {};
    setLocalFilters(emptyFilters);
    ownerSearchModal.setFilters(emptyFilters);
    onSearch(emptyFilters);
    toast.success("فیلترها پاک شدند");
  };

  const updateFilter = (key: keyof OwnerSearchFilters, value: any) => {
  ...
  return (
    <Dialog
      open={ownerSearchModal.isOpen}
      onOpenChange={(open) => {
        if (!open) {
          ownerSearchModal.onClose();
        }
      }}
    >
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="ttel">تلفن</Label>
                <Input
                  id="ttel"
                  placeholder="جستجو در تلفن"
                  value={localFilters.ttel || ""}
                  onChange={(e) => updateFilter("ttel", e.target.value)}
                />
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="storePelak">پلاک واحد</Label>
                <Input
                  id="storePelak"
                  placeholder="جستجو در پلاک واحد"
                  value={localFilters.storePelak || ""}
                  onChange={(e) => updateFilter("storePelak", e.target.value)}
                />
              </div>
              <div className="space-y-2">
                <Label htmlFor="changeOwner">تاریخ انتقال</Label>
                <Input
                  id="changeOwner"
                  placeholder="جستجو در تاریخ انتقال"
                  value={localFilters.changeOwner || ""}
                  onChange={(e) => updateFilter("changeOwner", e.target.value)}
                />
              </div>
            </div>
          </div>
          ...
          {/* Change Owner Date Range Filter */}
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-gray-700">فیلتر تاریخ انتقال</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="changeOwnerFrom">از تاریخ</Label>
                <DatePicker
                  value={localFilters.changeOwnerFrom || ""}
                  onChange={(date) =>
                    updateFilter("changeOwnerFrom", date ? date.toString() : undefined)
                  }
                  style={{
                    width: "100%",
                    height: "38px",
              ...
              <div className="space-y-2">
                <Label htmlFor="changeOwnerTo">تا تاریخ</Label>
                <DatePicker
                  value={localFilters.changeOwnerTo || ""}
                  onChange={(date) =>
                    updateFilter("changeOwnerTo", date ? date.toString() : undefined)
                  }
                  style={{
                    width: "100%",
                    height: "38px",
        ...
        <DialogFooter className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={ownerSearchModal.onClose}>
              انصراف
            </Button>
...
export default useOwnerSearchModal;
