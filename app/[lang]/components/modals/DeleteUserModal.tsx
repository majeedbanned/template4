...
export const DeleteUserModal = ({
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (data) => {
    //console.log("deleted >" + _DeleteUserModal.editID);
    const promise = () =>
      new Promise((resolve) => {
        setIsLoading(true);
        fetch(`/api/users/${_DeleteUserModal.editID}`, {
          method: "DELETE",
  ...
  return (
    <Modal
      disabled={isLoading}
      isOpen={_DeleteUserModal.isOpen}
      title="حذف اطلاعات"
      // "افزودن واحد"
      actionLabel="حذف"
      actionLabelVariant="destructive"
      secondaryActionLabel="انصراف"
      secondaryAction={_DeleteUserModal.onClose}
      onClose={_DeleteUserModal.onClose}
