"use client";
import { columns } from "@/app/[lang]/(admin)/admin/bazar/components/columns";
...
import useAddEditBazarModal, {
  AddEditBazarModal,
} from "@/app/[lang]/components/modals/AddEditBazarModal";
import useDeleteBazarModal, {
  DeleteBazarModal,
} from "@/app/[lang]/components/modals/DeleteBazarModal";
import { z } from "zod";
import { Bazarschema } from "@/lib/schemas";
...
export default function Datalist({
  ...
  const [editbazar, setEditbazar] = useState<z.infer<typeof Bazarschema>>();
  ...
  const AddBazarModal = useAddEditBazarModal();
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR<z.infer<typeof Bazarschema>[]>(
    `/api/bazar${searchParams ? `?${searchParams.toString()}` : ""}`,
    fetcher,
    {
  ...
  const handleDeleteClick = (rowData: any) => {
    const promise = () =>
      new Promise((resolve) => {
        ...
        setDelLable1(`عنوان  : ${rowData.bazar}`);
        setDelLable2(rowData.id);
        setTimeout(() => {
          _DeletebazarModal.onOpen(rowData.id);
          resolve("");
        }, 100);
      });
  ...
  const handleActionClick = (rowData: any) => {
    const promise = () =>
      new Promise((resolve) => {
        fetch("/api/bazar/" + (rowData.id !== "" ? rowData.id : "1")).then(
          async (res) => {
            if (res.status === 200) {
              ...
              setEditbazar(val);
              setTimeout(() => {
                AddBazarModal.onOpen(rowData.id);
                resolve("");
              }, 100);
            } else {
  ...
  return (
    <div>
      <DeleteBazarModal
        ...
        delLabel2={delLable2}
      ></DeleteBazarModal>
      <AddEditBazarModal mutation={mutate} data={editbazar}></AddEditBazarModal>
      <div className="flex flex-col p-2 py-4 text-slate-400 text-sm">
        <div>
          <div> اطلاعات پایه / تعریف بلوک</div>
        </div>
      ...
      {true ? (
        ...
        >
          ...
          <span className="text-white">بازار جدید</span>
        </Button>
      ) : (
